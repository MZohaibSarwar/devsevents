...
interface EventFormData {
  ...
  description: string;
  overview: string;
  // store the selected File; backend will accept multipart/form-data
  image: File | null;
  venue: string;
  location: string;
  date: string;
...
/**
 * EventForm Component
 * A comprehensive form to create new events with client-side validation
 */
export default function EventForm() {
  ...
  const handleInputChange = (
    ...
    // For file input we handle separately
  ...
  // Handle image file upload (separate from standard inputs)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Event "${responseData.event?.title}" created successfully!` });
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 bg-black rounded-xl shadow-lg">
      <h1 className="text-3xl font-semibold mb-6 text-white">Create Event</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full px-6 py-3 bg-green-500 text-white font-medium rounded-lg hover:bg-green-600 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
        >
